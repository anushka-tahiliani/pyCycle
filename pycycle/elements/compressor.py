...
class CorrectedInputsCalc(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        print(f"[DEBUG] {self.pathname} Wc={float(outputs['Wc']):.4f}, Nc={float(outputs['Nc']):.4f}, W={float(self.W):.4f}, theta={float(self.theta):.6f}, delta={float(self.delta):.6f}")
...
class Power(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['power'] = inputs['W'] * (inputs['ht_in'] - inputs['ht_out']) * BTU_s2HP
        print(f"W: {inputs['W']}, ht_in: {inputs['ht_in']}, ht_out: {inputs['ht_out']}, Δh: {inputs['ht_in'] - inputs['ht_out']}, BTU_s2HP: {BTU_s2HP}, power: {inputs['W'] * (inputs['ht_in'] - inputs['ht_out']) * BTU_s2HP}", flush = True)
        outputs['trq'] = HP_per_RPM_to_FT_LBF * outputs['power'] / inputs['Nmech']
...
class BleedsAndPower(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        for BN in self.options['bleed_names']:
            ...
            outputs[BN + ':Pt'] = Pt_in + inputs[BN + ':frac_P'] * (Pt_out - Pt_in)
            ht = ht_in + inputs[BN + ':frac_work'] * (ht_out - ht_in)
            print(f"ht_in: {ht_in}, ht_out: {ht_out}, frac_work ({BN}:frac_work): {inputs[BN + ':frac_work']}, Δh: {ht_out - ht_in}, ht: {ht_in + inputs[BN + ':frac_work'] * (ht_out - ht_in)}", flush=True)
...
class EnthalpyRise(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        print(f"[{self.pathname}] EnthalpyRise.compute() called", flush=True)
        inlet_ht = inputs['inlet_ht']
        outputs['ht_out'] = (inputs['ideal_ht'] - inlet_ht) / inputs['eff'] + inlet_ht
        print(f"ideal_ht: {inputs['ideal_ht']}, inlet_ht: {inlet_ht}, eff: {inputs['eff']}, ht_out: {(inputs['ideal_ht'] - inlet_ht) / inputs['eff'] + inlet_ht}", flush=True)
