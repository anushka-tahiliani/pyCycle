...
#         self.add_input('Ps_calc', val=2, units='lbf/inch**2', desc='Calculated exhaust static pressure')

#         self.add_output('PR', val=1.2, lower=1.000001, units=None, desc='Total-to-staic pressure ratio') #this value needs to be changed to Nozzle PR guess
...
#         self.add_input('PR', val=2, desc='Total-to-staic pressure ratio') #this value needs to be changed to Nozzle PR guess
...
#         self.add_output('PR', val=1.2, lower=1.000001, units=None, desc='Total-to-staic pressure ratio')
...
#         # inputs
#         self.add_input('Pt_in', val=10.0, units='lbf/inch**2', desc='Entrance total pressure')
#         self.add_input('PR', val=1.2, desc='Total-to-static pressure ratio') #this one matters
#         self.add_input('dPqP', val=0.0, desc='Total pressure loss from inlet to throat')
...
#FIXED NOZZLE WITH INITIAL GUESS/PR_GUESS
""" Class definition for Nozzle."""
...
class PR_bal(om.ImplicitComponent):
    ...
    def initialize(self):
        self.options.declare('PR_guess', default=2.0, desc='initial guess for PR in balance')

    def setup(self):
        self.add_input('Ps_exhaust', val=5.0, units='lbf/inch**2', desc='Exhaust static pressure')
        self.add_input('Ps_calc', val=5.0, units='lbf/inch**2', desc='Calculated exhaust static pressure')

        self.add_output('PR', val=self.options['PR_guess'], lower=1.000001, units=None, desc='Total-to-staic pressure ratio')
...
class Nozzle(Element):
    ...
    def initialize(self):
        ...
        self.options.declare('lossCoef', default='Cv',
                              desc='If set to "Cfg", then Gross Thrust Coefficient is an input.')
        self.options.declare('internal_solver', default=False)
        self.options.declare('PR_guess', default=2.0)
    ...
    def setup(self):
        ...
        nozzType = self.options['nozzType']
        lossCoef = self.options['lossCoef']
        PR_guess = self.options['PR_guess']
        ...
        self.add_subsystem('PR_bal', PR_bal(PR_guess=PR_guess), promotes_inputs=['*'], promotes_outputs=['*'] )
