    ...
    def compute(self, inputs, outputs):
        ...
        def fmt(x):
            try:
                ...
                return np.array2string(x, precision=6, floatmode='fixed')


        # # single‐line debug print
        # print(
        #     f"[DEBUG] {self.pathname} | "
        #     f"Wc_actual={fmt(inputs['Wc_actual'])}  Wc_SMN={fmt(inputs['Wc_SMN'])}  SMN={fmt(outputs['SMN'])}  |  "
        #     f"PR_actual={fmt(inputs['PR_actual'])}  PR_SMN={fmt(inputs['PR_SMN'])}  |  "
        #     f"PR_SMW={fmt(inputs['PR_SMW'])}  SMW={fmt(outputs['SMW'])}"
        # )
