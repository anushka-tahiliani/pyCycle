...
class Shaft(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        print(f"[DEBUG] {self.pathname} - "
            f"Nmech={float(Nmech):.6f},trq_in={float(trq_in):.6f}, trq_out={float(trq_out):.6f}, trq_net={float(trq_net):.6f}, "
            f"pwr_in={float(outputs['pwr_in']):.6f}, pwr_out={float(outputs['pwr_out']):.6f}, "
            f"pwr_net={float(outputs['pwr_net']):.6f}, pwr_in_real={float(outputs['pwr_in_real']):.6f}, "
            f"pwr_out_real={float(outputs['pwr_out_real']):.6f}")
