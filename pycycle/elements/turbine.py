...
#             print(f"[DEBUG] {self.pathname} compute - Tt={inputs['Tt']}, Pt={inputs['Pt']}, W_in={inputs['W_in']}, Nmech={inputs['Nmech']}, Wp={outputs['Wp']}, Np={outputs['Np']}")
#         except FloatingPointError:
#             raise AnalysisError('Bad values flow states in {}: T={}  P={}'.format(self.pathname, inputs['Tt'], inputs['Pt']))
...
#     def compute(self, inputs, outputs):
#         print(f"[DEBUG] {self.pathname} compute start")
#         PR     = inputs['PR']
#         print(f"PR={PR}")
#         S_in   = inputs['S_in']
#         S_out  = inputs['S_out']
...
#         print(f"[DEBUG] {self.pathname} - PR={PR}, S_in={S_in}, S_out={S_out}, Rt={Rt}, invPR={invPR}")
...
#         print(f"[DEBUG] {self.pathname} - Pt_in={inputs['Pt_in']}, PR={inputs['PR']}, Pt_out={outputs['Pt_out']}")
...
#         print(f"[DEBUG] {self.pathname} - EnthalpyDrop")
#         print(f"         ht_in = {inputs['ht_in']:.6f} Btu/lbm, ht_out_ideal = {inputs['ht_out_ideal']:.6f} Btu/lbm, eff = {inputs['eff']:.6f}")
#         print(f"         ht_out = {outputs['ht_out']:.6f} Btu/lbm")
...
#         print(f"[DEBUG] Pt_out={float(Pt_out):.6f}, delta_Pt={float(delta_Pt):.6f}")

#         bleeds = self.options['bleed_names']
#         for BN in bleeds:
#             outputs[BN + ':Pt'] = Pt_out + inputs[BN+':frac_P'] * delta_Pt

#             print(f"[DEBUG BLEED Pt] BN={BN}, Pt_out={float(Pt_out):.6f}, "
#                 f"frac_P={float(inputs[BN+':frac_P']):.6f}, delta_Pt={float(delta_Pt):.6f}, "
#                 f"{BN}:Pt={float(outputs[BN + ':Pt']):.6f}")
...
#         print(f"[DEBUG] ht_in={float(ht_in):.6f}, ht_out_ideal={float(ht_out_ideal):.6f}, eff={float(eff):.6f}, "
#             f"W_in={float(W_in):.6f}, W_out={float(W_out):.6f}, "
#             f"ht_out_b4bld={float(ht_out_b4bld):.6f}, ht_out={float(ht_out):.6f}, power={float(power):.6f}")
...
#             print(f"[DEBUG BLEED] BN_W={BN_W}, BN_ht={BN_ht}, BN_ht_ideal={BN_ht_ideal}, "
#                 f"W={float(W):.6f}, ht={float(ht):.6f}, ht_ideal={float(ht_ideal):.6f}, eff={float(eff):.6f}, "
#                 f"ht_out={float(ht_out):.6f}, power={float(power):.6f}")
